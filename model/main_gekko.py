...
m.Equations([m.regional_emissions[i] <= m.baseline_emissions[i] for i in regions_i])
...
for i, region in enumerate(regions):
    regional_damage_factor = regions[region].get('damage factor', 1)
    m.damage_costs[i]       = m.Intermediate( regional_damage_factor * (economics.damage_fct(m.temperature) - economics.damage_fct(T0)) )
...
for i, region in enumerate(regions):
    ...
    m.Equation(m.GDP_net[i] > 0.3 * m.GDP_gross[i])
...
if m.options.SOLVESTATUS:
    ...
    print('NPV', m.NPV.value[-1])
else:
    print("Optimal solution NOT found.")
print('Took {} seconds'.format(timer_1-timer_0))

# Create plot
full_plot(m, regions, years, params)