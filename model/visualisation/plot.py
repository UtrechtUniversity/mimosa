...
def full_plot(m, baseline):
    ...
    plot.add(lambda t,r: baseline(t+m.beginyear, r), row=1, is_fct=True, name='baseline_emissions')
    ...
    plot.add(m.global_emissions, False, row=3)
    plot.add(m.temperature, False, row=3, secondary_y=True)
    plot.add(m.cumulative_emissions, False, row=3, visible='legendonly')
