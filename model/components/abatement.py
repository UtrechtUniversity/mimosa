...
def constraints(m):
    ...
    constraints.extend([
        GlobalConstraint(lambda m,t: m.global_rel_abatement_costs[t] == sum(m.abatement_costs[t,r] for r in m.regions) / sum(m.GDP_gross[t,r] for r in m.regions), 'global_rel_abatement_costs')
    ])
