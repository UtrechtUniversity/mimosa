...
def constraints(m):
    ...
    m.capital_stock = Var(
        m.t, m.regions, initialize=lambda m, t, r: m.init_capitalstock[r]
    )
    ...
    m.consumption = Var(
        m.t, m.regions, initialize=lambda m: (1 - m.sr) * m.GDP[0, m.regions.first()]
    )
    ...
    regional_constraints.extend(
        [
            ...
            == m.GDP_gross[t, r] * (1 - m.damage_costs[t, r]) - m.abatement_costs[t, r],
            ...
            == (soft_min(m.consumption[t, r] / m.L[t, r]) ** (1 - m.elasmu) - 1)
            / (1 - m.elasmu)
            - 1,
            lambda m, t, r: (
                m.capital_stockdot[t, r]
                == economics.calc_dKdt(
    ...
    global_constraints.extend(
        [
            lambda m, t: m.consumption_NPVdot[t]
            == sum(exp(-0.05 * t) * m.consumption[t, r] for r in m.regions),
            lambda m, t: m.baseline_consumption_NPVdot[t]
            == sum(exp(-0.05 * t) * (1 - m.sr) * m.GDP[t, r] for r in m.regions),
            lambda m, t: m.consumption_loss[t]
            == 1 - m.consumption_NPV[t] / m.baseline_consumption_NPV[t]
            if t > 0
            else Constraint.Skip,
        ]
    )
    ...
    m.relative_damage_factor = Param()

    regional_constraints.extend(
        [
            lambda m, t, r: m.damage_costs[t, r] <= m.relative_damage_factor
            if (t >= value(m.year2100) and value(m.relative_damage_factor) is not False)
            else Constraint.Skip
        ]
    )
