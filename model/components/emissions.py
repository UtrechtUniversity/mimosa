...
def constraints(m):
    ...
    regional_constraints.extend(
        [
            ...
            == (1 - m.relative_abatement[t, r]) * m.baseline_emissions[t, r],
        ]
    )
    ...
    global_constraints.append(
        lambda m, t: m.cumulative_emissionsdot[t] == m.global_emissions[t]
    )

    m.T0 = Param()
    m.temperature = Var(m.t, initialize=lambda m, t: m.T0)
    m.temperaturedot = DerivativeVar(m.temperature, wrt=m.t, bounds=(-2, 2))
    m.TCRE = Param()
    global_constraints.append(
    ...
    global_constraints.extend(
        [
            lambda m, t: m.cumulative_emissions[t] - m.budget <= 0
            if (t >= value(m.year2100) and value(m.budget) is not False)
            else Constraint.Skip,
            lambda m, t: m.cumulative_emissions[t] >= 0,
            lambda m, t: m.global_emissionsdot[t]
            >= m.inertia_global * sum(m.baseline_emissions[0, r] for r in m.regions)
            if value(m.inertia_global) is not False
            else Constraint.Skip,
            lambda m, t: m.global_emissions[t] >= m.min_level
            if value(m.min_level) is not False
            else Constraint.Skip,
    ...
    regional_constraints.append(
        lambda m, t, r: m.regional_emissionsdot[t, r]
        >= m.inertia_regional * m.baseline_emissions[0, r]
    ...
    global_constraints.append(
        lambda m, t: (
            ...
            / sum(m.baseline_cumulative(t, r) for r in m.regions)
        )
        if t > 0
        else Constraint.Skip
    )
