"""Compare MIMOSA sea-level rise in 2100 with IPCC AR6 estimates.
...
def run_temperature_benchmark(model, final_temperature):
    ...
    for t in model.t:
        ...
        glaciers = value(
            sealevelrise.slr_gsic(glaciers, temperature, model, period_length)
        )
        greenland = value(
            sealevelrise.slr_gis(greenland, temperature, model, period_length)
        )
...
def calculate_benchmark_tables():
    """Calculate total SLR and central-projection component tables."""
...
def write_benchmark_tables():
    """Write the benchmark tables used by the SLR documentation."""
