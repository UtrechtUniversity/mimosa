    ...
    constraints = [
        ...
        GlobalEquation(
            m.tipping_temps_PFAT,
             lambda m, t: (
                   calc_temp_PFAT(m.PFAT_tipped, m.PFAT_threshold, m.PFAT_tip_year, m.temperature[t],
                                  m.temperature[t-1], m)
             )
        ),
...
def calc_temp_PFAT(PFAT_tipped, PFAT_threshold, PFAT_tip_year, temp_current, temp_prev, m: AbstractModel):
