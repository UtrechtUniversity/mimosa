...
#####################################################
    constraints = [
        ...
        GlobalEquation(
            ...
            lambda m, t: (calc_global_temp_LABC(m.include_LABC, m.LABC_threshold_quantile, 
                                                m.temperature[t], m,))
            if t > 0
            else 0
        ),
        ...
        GlobalEquation(
            ...
            lambda m, t: (calc_global_temp_AMOC(m.include_AMOC, m.AMOC_threshold_quantile, 
                                                m.temperature[t], m,))
            if t > 0
            else 0
        ),
        ...
        GlobalEquation(
            ...
            lambda m, t: (calc_global_temp_AMAZ(m.include_AMAZ, m.AMAZ_threshold_quantile, m.temperature[t],
                                                m.AMAZ_severity_quantile, m,))
            if t > 0
            else 0
        ),
...
def calc_global_temp_LABC(include_LABC, LABC_threshold_quantile, temp_current, m: AbstractModel):

    # if LABC is set to False, don't do anything
    # TODO: It still does something...Too bad!
    if (include_LABC == False):
        return 0
...
def calc_global_temp_AMOC(include_AMOC, AMOC_threshold_quantile, temp_current, m: AbstractModel):

    # if AMOC is set to False, don't do anything
    # TODO: It still does something...Too bad!
    if (include_AMOC == False):
        return 0
...
def calc_global_temp_AMAZ(include_AMAZ, AMAZ_threshold_quantile, temp_current, AMAZ_severity_quantile, m: AbstractModel):

    # if AMAZ is set to False, don't do anything
    # TODO: It still does something...Too bad!
    if (include_AMAZ == False):
        return 0
